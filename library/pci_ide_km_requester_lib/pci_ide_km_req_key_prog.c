 ...
 **/
libspdm_return_t pci_ide_km_key_prog(const void *pci_doe_context,
                                     ...
                                     uint8_t *kp_ack_status)
{
    ...
    pci_ide_km_key_prog_mine_t request;
    size_t request_size;
    ...
    libspdm_zero_mem (&request.key_buffer, sizeof(pci_ide_km_aes_256_gcm_key_buffer_t));
    if (LIBSPDM_STATUS_IS_ERROR(status)) {
        return status;
    }

    if (response_size != sizeof(pci_ide_km_kp_ack_t)) {
        return LIBSPDM_STATUS_INVALID_MSG_SIZE;
    }
    if (response.header.object_id != PCI_IDE_KM_OBJECT_ID_KP_ACK) {
