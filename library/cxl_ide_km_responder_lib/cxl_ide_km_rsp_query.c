...
{
    const cxl_ide_km_query_t *ide_km_request;
    cxl_ide_km_query_resp_mine_t *ide_km_response;
    ...
    if (request_size != sizeof(cxl_ide_km_query_t)) {
        return LIBSPDM_STATUS_INVALID_MSG_SIZE;
    }
    LIBSPDM_ASSERT (*response_size >= sizeof(cxl_ide_km_query_resp_mine_t));
    *response_size = sizeof(cxl_ide_km_query_resp_mine_t);
    ...
    status = cxl_ide_km_device_query (pci_doe_context, spdm_context, session_id,
                                      ...
                                      &ide_km_response->caps,
                                      &ide_reg_buffer, &ide_reg_buffer_count);
    if (LIBSPDM_STATUS_IS_ERROR(status)) {
        return status;
    }
    LIBSPDM_ASSERT (ide_reg_buffer_count <= CXL_IDE_KM_IDE_CAP_REG_BLOCK_MAX_COUNT);
    libspdm_copy_mem (ide_km_response->ide_reg_block,
                      sizeof(ide_km_response->ide_reg_block),
                      ide_reg_buffer,
                      ide_reg_buffer_count * sizeof(uint32_t));

    *response_size = sizeof(cxl_ide_km_query_resp_t) + ide_reg_buffer_count * sizeof(uint32_t);
