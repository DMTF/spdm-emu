 ...
 **/
libspdm_return_t pci_ide_km_get_response_key_set_stop (const void *pci_doe_context,
                                                       const void *spdm_context, const uint32_t *session_id,
                                                       const void *request, size_t request_size,
                                                       void *response, size_t *response_size)
{
    const pci_ide_km_k_set_stop_t *ide_km_request;
    pci_ide_km_k_gostop_ack_t *ide_km_response;
    libspdm_return_t status;

    ide_km_request = request;
    ide_km_response = response;
    if (request_size != sizeof(pci_ide_km_k_set_stop_t)) {
        return LIBSPDM_STATUS_INVALID_MSG_SIZE;
    }
    LIBSPDM_ASSERT (*response_size >= sizeof(pci_ide_km_k_gostop_ack_t));
    *response_size = sizeof(pci_ide_km_k_gostop_ack_t);

    libspdm_zero_mem (response, *response_size);
    ide_km_response->header.object_id = PCI_IDE_KM_OBJECT_ID_K_SET_GOSTOP_ACK;
    ...
    status = pci_ide_km_device_key_set_stop (pci_doe_context, spdm_context, session_id,
