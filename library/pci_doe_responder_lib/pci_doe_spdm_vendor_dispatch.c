    ...
    void *response, uintn *response_size)
{
    ...
    if (spdm_request->pci_doe_vendor_header.len != sizeof(spdm_request->pci_doe_vendor_header.vendor_id)) {
        return RETURN_INVALID_PARAMETER;
    }
    ...
    for (index = 0; index < ARRAY_SIZE(m_pci_doe_spdm_dispatch); index++) {
        if (spdm_request->pci_doe_vendor_header.pci_protocol.protocol_id == m_pci_doe_spdm_dispatch[index].header.protocol_id) {
            ...
            spdm_response->pci_doe_vendor_header.len = sizeof(spdm_response->pci_doe_vendor_header.vendor_id);
            spdm_response->pci_doe_vendor_header.vendor_id = SPDM_VENDOR_ID_PCISIG;
            spdm_response->pci_doe_vendor_header.payload_length = (uint16_t)(sizeof(pci_protocol_header_t) + vendor_response_size);
            spdm_response->pci_doe_vendor_header.pci_protocol.protocol_id = spdm_request->pci_doe_vendor_header.pci_protocol.protocol_id;
