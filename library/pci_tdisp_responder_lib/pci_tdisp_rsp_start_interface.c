...
{
    const pci_tdisp_start_interface_request_t *tdisp_request;
    pci_tdisp_start_interface_response_t *tdisp_response;
    libtdisp_error_code_t error_code;

    tdisp_request = request;
    tdisp_response = response;
    if (request_size != sizeof(pci_tdisp_start_interface_request_t)) {
    ...
    LIBSPDM_ASSERT (*response_size >= sizeof(pci_tdisp_start_interface_response_t));
    *response_size = sizeof(pci_tdisp_start_interface_response_t);

    libspdm_zero_mem (response, *response_size);
    tdisp_response->header.version = tdisp_request->header.version;
    tdisp_response->header.message_type = PCI_TDISP_START_INTERFACE_RSP;
    tdisp_response->header.interface_id.function_id = tdisp_request->header.interface_id.function_id;

    error_code = pci_tdisp_device_start_interface (pci_doe_context, spdm_context, session_id,
                                                   &tdisp_response->header.interface_id,
                                                   tdisp_request->start_interface_nonce);
