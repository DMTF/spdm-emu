                                     ...
                                     uint8_t *kp_ack_status)
{
    ...
    cxl_ide_km_key_prog_mine_t request;
    size_t request_size;
    ...
    if (response_size != sizeof(cxl_ide_km_kp_ack_t)) {
        return LIBSPDM_STATUS_INVALID_MSG_SIZE;
    }
    if (response.header.object_id != CXL_IDE_KM_OBJECT_ID_KP_ACK) {
