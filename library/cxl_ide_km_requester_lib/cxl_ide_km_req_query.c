                                  ...
                                  uint32_t *ide_reg_buffer, uint32_t *ide_reg_buffer_count)
{
    ...
    size_t request_size;
    cxl_ide_km_query_resp_mine_t response;
    ...
    if (response_size < sizeof(cxl_ide_km_query_resp_t)) {
        return LIBSPDM_STATUS_INVALID_MSG_SIZE;
    }
    ide_reg_count = (uint32_t)(response_size - sizeof(cxl_ide_km_query_resp_t)) / sizeof(uint32_t);
    if (response_size != sizeof(cxl_ide_km_query_resp_t) + ide_reg_count * sizeof(uint32_t)) {
        return LIBSPDM_STATUS_INVALID_MSG_SIZE;
    }

    if (response.header.object_id != CXL_IDE_KM_OBJECT_ID_QUERY_RESP) {
