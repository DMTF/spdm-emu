                                                 ...
                                                 bool need_session)
{
    ...
    if (((rsp_cap_flags & SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_CERT_CAP) == 0) ||
        ...
        ((rsp_cap_flags & SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_HBEAT_CAP) == 0) ) {
        return false;
    }
...
void spdm_test_case_heartbeat_ack_success_11_dhe (void *test_context)
{
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_HEARTBEAT_ACK,
        SPDM_RESPONDER_TEST_CASE_HEARTBEAT_ACK_SUCCESS_11_IN_DHE_SESSION, 1,
        test_result, "response size - %d", spdm_response_size);
    if (test_result == COMMON_TEST_RESULT_FAIL) {
        return;
    }

    if (spdm_response->header.request_response_code == SPDM_HEARTBEAT_ACK) {
        test_result = COMMON_TEST_RESULT_PASS;
    } else {
        test_result = COMMON_TEST_RESULT_FAIL;
    }
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_HEARTBEAT_ACK,
        SPDM_RESPONDER_TEST_CASE_HEARTBEAT_ACK_SUCCESS_11_IN_DHE_SESSION, 2,
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_HEARTBEAT_ACK,
        SPDM_RESPONDER_TEST_CASE_HEARTBEAT_ACK_SUCCESS_11_IN_DHE_SESSION, 3,
...
void spdm_test_case_heartbeat_ack_version_mismatch (void *test_context)
{
    ...
    for (index = 0; index < LIBSPDM_ARRAY_SIZE(mismatched_version); index++) {
        ...
        common_test_record_test_assertion (
            SPDM_RESPONDER_TEST_GROUP_HEARTBEAT_ACK,
            SPDM_RESPONDER_TEST_CASE_HEARTBEAT_ACK_VERSION_MISMATCH_IN_DHE_SESSION, 1,
            test_result, "response size - %d", spdm_response_size);
        if (test_result == COMMON_TEST_RESULT_FAIL) {
            continue;
        }
        ...
        common_test_record_test_assertion (
            SPDM_RESPONDER_TEST_GROUP_HEARTBEAT_ACK,
            SPDM_RESPONDER_TEST_CASE_HEARTBEAT_ACK_VERSION_MISMATCH_IN_DHE_SESSION, 2,
            test_result, "response code - 0x%02x", spdm_response->header.request_response_code);
        if (test_result == COMMON_TEST_RESULT_FAIL) {
            continue;
        }
        ...
        common_test_record_test_assertion (
            SPDM_RESPONDER_TEST_GROUP_HEARTBEAT_ACK,
            SPDM_RESPONDER_TEST_CASE_HEARTBEAT_ACK_VERSION_MISMATCH_IN_DHE_SESSION, 3,
            test_result, "response version - 0x%02x", spdm_response->header.spdm_version);
        if (test_result == COMMON_TEST_RESULT_FAIL) {
            continue;
        }
        ...
        common_test_record_test_assertion (
            SPDM_RESPONDER_TEST_GROUP_HEARTBEAT_ACK,
            SPDM_RESPONDER_TEST_CASE_HEARTBEAT_ACK_VERSION_MISMATCH_IN_DHE_SESSION, 4,
        ...
        common_test_record_test_assertion (
            SPDM_RESPONDER_TEST_GROUP_HEARTBEAT_ACK,
            SPDM_RESPONDER_TEST_CASE_HEARTBEAT_ACK_VERSION_MISMATCH_IN_DHE_SESSION, 5,
            test_result, "response param2 - 0x%02x", spdm_response->header.param2);
    }
}

void spdm_test_case_heartbeat_ack_unexpected_request (void *test_context)
{
    spdm_test_context_t *spdm_test_context;
    void *spdm_context;
    libspdm_return_t status;
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_HEARTBEAT_ACK,
        SPDM_RESPONDER_TEST_CASE_HEARTBEAT_ACK_UNEXPECTED_REQUEST_IN_DHE_SESSION_HS, 1,
        test_result, "response size - %d", spdm_response_size);
    if (test_result == COMMON_TEST_RESULT_FAIL) {
        return;
    }
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_HEARTBEAT_ACK,
        SPDM_RESPONDER_TEST_CASE_HEARTBEAT_ACK_UNEXPECTED_REQUEST_IN_DHE_SESSION_HS, 2,
        test_result, "response code - 0x%02x", spdm_response->header.request_response_code);
    if (test_result == COMMON_TEST_RESULT_FAIL) {
        return;
    }
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_HEARTBEAT_ACK,
        SPDM_RESPONDER_TEST_CASE_HEARTBEAT_ACK_UNEXPECTED_REQUEST_IN_DHE_SESSION_HS, 3,
        test_result, "response version - 0x%02x", spdm_response->header.spdm_version);
    if (test_result == COMMON_TEST_RESULT_FAIL) {
        return;
    }
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_HEARTBEAT_ACK,
        SPDM_RESPONDER_TEST_CASE_HEARTBEAT_ACK_UNEXPECTED_REQUEST_IN_DHE_SESSION_HS, 4,
        test_result, "response param1 - 0x%02x", spdm_response->header.param1);
    if (test_result == COMMON_TEST_RESULT_FAIL) {
        return;
    }
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_HEARTBEAT_ACK,
        SPDM_RESPONDER_TEST_CASE_HEARTBEAT_ACK_UNEXPECTED_REQUEST_IN_DHE_SESSION_HS, 5,
        test_result, "response param2 - 0x%02x", spdm_response->header.param2);
}

void spdm_test_case_heartbeat_ack_session_required (void *test_context)
{
    spdm_test_context_t *spdm_test_context;
    void *spdm_context;
    libspdm_return_t status;
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_HEARTBEAT_ACK,
        SPDM_RESPONDER_TEST_CASE_HEARTBEAT_ACK_SESSION_REQUIRED, 1,
        test_result, "response size - %d", spdm_response_size);
    if (test_result == COMMON_TEST_RESULT_FAIL) {
        return;
    }
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_HEARTBEAT_ACK,
        SPDM_RESPONDER_TEST_CASE_HEARTBEAT_ACK_SESSION_REQUIRED, 2,
        test_result, "response code - 0x%02x", spdm_response->header.request_response_code);
    if (test_result == COMMON_TEST_RESULT_FAIL) {
        return;
    }
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_HEARTBEAT_ACK,
        SPDM_RESPONDER_TEST_CASE_HEARTBEAT_ACK_SESSION_REQUIRED, 3,
        test_result, "response version - 0x%02x", spdm_response->header.spdm_version);
    if (test_result == COMMON_TEST_RESULT_FAIL) {
        return;
    }
    ...
    common_test_record_test_assertion (
        SPDM_RESPONDER_TEST_GROUP_HEARTBEAT_ACK,
        SPDM_RESPONDER_TEST_CASE_HEARTBEAT_ACK_SESSION_REQUIRED, 4,
        test_result, "response param1 - 0x%02x", spdm_response->header.param1);
    if (test_result == COMMON_TEST_RESULT_FAIL) {
        return;
    }
