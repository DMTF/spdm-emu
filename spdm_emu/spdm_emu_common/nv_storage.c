					 ...
					 IN boolean is_requester)
{
	...
	if (m_use_version == SPDM_MESSAGE_VERSION_11) {
		...
		spdm_set_data(spdm_context, SPDM_DATA_DHE_NAME_GROUP,
			      &parameter, &data16, sizeof(data16));
		data16 = m_support_aead_algo;
		spdm_set_data(spdm_context, SPDM_DATA_AEAD_CIPHER_SUITE,
			      &parameter, &data16, sizeof(data16));
		data16 = m_support_req_asym_algo;
		spdm_set_data(spdm_context, SPDM_DATA_REQ_BASE_ASYM_ALG,
			      &parameter, &data16, sizeof(data16));
		data16 = m_support_key_schedule_algo;
		spdm_set_data(spdm_context, SPDM_DATA_KEY_SCHEDULE, &parameter,
	...
	spdm_set_data(spdm_context, SPDM_DATA_CONNECTION_STATE, &parameter,
		      &data32, sizeof(data32));
					 ...
					 IN boolean is_requester)
{
	...
	if (is_requester) {
		...
		spdm_get_data(spdm_context, SPDM_DATA_CAPABILITY_FLAGS,
			      &parameter, &data32, &data_size);
		negotiated_state.requester_cap_flags = data32;
		data_size = sizeof(data8);
		spdm_get_data(spdm_context, SPDM_DATA_CAPABILITY_CT_EXPONENT,
	...
	spdm_get_data(spdm_context, SPDM_DATA_SPDM_VERSION, &parameter,
	...
	data_size = sizeof(data8);
	spdm_get_data(spdm_context, SPDM_DATA_MEASUREMENT_SPEC, &parameter,
		      &data8, &data_size);
	...
	data_size = sizeof(data16);
	spdm_get_data(spdm_context, SPDM_DATA_AEAD_CIPHER_SUITE, &parameter,
		      &data16, &data_size);
	...
	data_size = sizeof(data16);
	spdm_get_data(spdm_context, SPDM_DATA_KEY_SCHEDULE, &parameter, &data16,
		      &data_size);
