...
void *spdm_server_init(void)
{
	...
	ASSERT((data32 & SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_CERT_CAP) == 0);
	#endif // !SPDM_ENABLE_CAPABILITY_CERT_CAP

	#if !SPDM_ENABLE_CAPABILITY_CHAL_CAP
	ASSERT((data32 & SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_CHAL_CAP) == 0);
	#endif // !SPDM_ENABLE_CAPABILITY_CHAL_CAP

	#if !SPDM_ENABLE_CAPABILITY_MEAS_CAP
	ASSERT((data32 & SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_MEAS_CAP) == 0);
	ASSERT((data32 & SPDM_GET_CAPABILITIES_RESPONSE_FLAGS_MEAS_FRESH_CAP) == 0);
