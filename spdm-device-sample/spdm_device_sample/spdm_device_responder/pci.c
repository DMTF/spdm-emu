...
void pci_cfg_write_32 (uint8_t bus, uint8_t device, uint8_t function, uint32_t offset, uint32_t value)
{
    size_t address;
    address = SPDM_DEVICE_PCIE_ADDRESS + PCI_ECAM_ADDRESS(bus, device, function, offset);
...
{
    return pci_cfg_read_32 (SPDM_DEVICE_PCI_BUS, SPDM_DEVICE_PCI_DEVICE, SPDM_DEVICE_PCI_FUNCTION, offset);
}

void spdm_dev_pci_cfg_write_32 (uint32_t offset, uint32_t value)
{
    pci_cfg_write_32 (SPDM_DEVICE_PCI_BUS, SPDM_DEVICE_PCI_DEVICE, SPDM_DEVICE_PCI_FUNCTION, offset, value);
}
